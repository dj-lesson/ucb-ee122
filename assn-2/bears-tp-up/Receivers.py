...
class Connection():
    def __init__(self, host, port, start_seq, debug=False):
        ...
        self.port = port
        self.max_buf_size = 5
        # (Author: AnDJ) Name format of the received file.
...
class Receiver():
    def __init__(self, listenport=33122, debug=False, timeout=10):
        ...
        # (Author: AnDJ) Some settings for this socket server.
    ...
    def start(self):
        ...
        while True:
            try:
                print 'AnDJ-Receiver: ready to receive'
                message, address = self.receive()
                msg_type, seqno, data, checksum = self._split_message(message)
                try:
                    seqno = int(seqno)
                    print 'AnDJ-Receiver: receive packet: %s|%d|%s' % (msg_type, seqno, checksum)
                except:
                    raise ValueError
                if debug:
                    print "%s %d %s %s" % (msg_type, seqno, data, checksum)
                if Checksum.validate_checksum(message):
                    print 'AnDJ-Receiver: will do with this received packet'
    ...
    # (Author: AnDJ) The message format is "ack|seqno|checksum"
    def _send_ack(self, seqno, address):
        print 'AnDJ-Receiver: will send the ack'
        m = "ack|%d|" % seqno
    ...
    def _handle_data(self, seqno, data, address):
        print 'AnDJ-Receiver: will handle the data packet'
